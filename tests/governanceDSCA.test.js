...
async function runAndSummarize(text) {
  ...
  const cycles = socket.emitted.filter((e) => e.event === "telemetry" && e.payload?.type === "cycle-update");
  const plan = socket.emitted.find((e) => e.event === "telemetry" && e.payload?.type === "cycle-plan");
  ...
  const finalOutputs = socket.emitted
    .filter((e) => e.event === "telemetry" && e.payload?.type === "final-output")
